...
(function(){
    ...
    // Call updatePagination(currentPage, totalPages)
    function update(currentPage, totalPages){
      total = Math.max(1, Math.floor(totalPages));
      current = Math.min(Math.max(1, Math.floor(currentPage)), total);
  
      currentEl.textContent = pad(current);
      totalEl.textContent = pad(total);
  
      // compute percent: first page -> 0%, last page -> 100%
      let percent = 0;
      if (total > 1){
        percent = ((current - 1) / (total - 1)) * 100;
      } else {
        percent = 100;
      }
      progressEl.style.width = percent + '%';
  
      // accessibility attributes (optional)
    ...
    document.getElementById('prev').addEventListener('click', ()=> update(current-1, total));
    document.getElementById('next').addEventListener('click', ()=> update(current+1, total));
    document.getElementById('goto').addEventListener('click', ()=> update(4, total));
  
    // initial render
    update(current, total);
  
    // expose globally if you want to call from outside
    window.updatePagination = update;
  })();